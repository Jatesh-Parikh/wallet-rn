...
export async function createTransaction(req, res) {
  try {
    ...
    const transaction =
      await sql`INSERT INTO transactions(user_id, title, amount, category) VALUES (${userId}, ${title}, ${amount}, ${category})`;

    res.status(201).json(transaction[0]);
  } catch (error) {
    console.log("Error creating the transaction:", error.message);
    res.status(500).json({ message: "Internal server error" });
  }
}
...
export async function getSummaryByUserId(req, res) {
  try {
    ...
    console.log("Error getting the summary:", error.message);
    res.status(500).json({ message: "Internal server error" });
  }
}
